...
int main() {
    try {
        ...
        std::unique_ptr<YOLOv5> algo3 = factory->create(Algo_Type::OpenCV);
        algo3->init("model_path3333333", Device_Type::CPU, Model_Type::FP16);
        algo3->infer("image_path33333");

        // 尝试创建未注册的算法类型
        auto algo4 = factory->create(static_cast<Algo_Type>(99)); // 应该抛出异常
