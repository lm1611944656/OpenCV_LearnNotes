...
int main(int argc, char** argv)
{
	...
	std::ifstream ifs("data/classs.txt");
	std::string line;
	...
	cv::Mat image = cv::imread("data/zidane.jpg"), blob;
	cv::Vec4d params;
	pre_process(image, blob, params);

	cv::dnn::Net net = cv::dnn::readNet("weigths/yolov5s-seg.onnx");
	std::vector<cv::Mat> outputs;
	process(blob, net, outputs);

	cv::Mat result = post_process(image, outputs, class_name, params);
