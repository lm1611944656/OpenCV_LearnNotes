...
std::map<std::string, CameraMetrics> CameraManager::getAllMetrics() 
{
    std::map<std::string, CameraMetrics> metrics_map;
    std::lock_guard<std::mutex> lock(mtx_);
    for (auto& [cam_id, camera] : cameras_) {
        metrics_map[cam_id] = camera->getMetrics();
    }
    return metrics_map;
}

void CameraManager::printAllMetrics()
{
    auto metrics = getAllMetrics();
    for (const auto& kv : metrics) {
        const std::string& cam_id = kv.first;
        const CameraMetrics& m = kv.second;
        std::cout << "[METRICS] Camera " << cam_id << ": " << m.toString() << std::endl;
    }
}
